...
export const generateCohortExcel = (
  ...
  const cohortRows = stats.cohorts.map(row => {
    const obj: any = { 
        ...
        'Tamanho (Iniciados)': row.totalStarters 
    };
    row.retention.forEach((val, idx) => {
      obj[`Mês ${idx}`] = val;
    });
    obj['Média'] = row.average;
    obj['Crescimento'] = row.growth;
    return obj;
  });
  const wsCohort = XLSX.utils.json_to_sheet(cohortRows);
  XLSX.utils.book_append_sheet(wb, wsCohort, "Análise de Cohort");
  ...
  XLSX.utils.book_append_sheet(wb, wsPercent, "Retenção %");

  XLSX.writeFile(wb, `Analise_Cohort_AdvEasy_${new Date().getTime()}.xlsx`);
};
